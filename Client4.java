...
public class Client4 {
	...
	Socket requestSocket2; 	// Socket to request chunks from Client3
	...
	void receivefromserver() // to receive chunks from Server
	{
		try {
			...
			try {
				...
				System.out.println("All Chunks corresponding to Client4 have been received from Server");
			}
	...
	void receivefromclient3() // to receive chunks from Client3
	{
		try {
			// create a socket to connect to the Client3
			requestSocket2 = new Socket("localhost", 9003);
			System.out.println("Connected to localhost Client 3 in port 9003 for 1st session.");
			...
			try {
				...
				System.out.println("Chunks received from Client3");
			}

		} catch (ConnectException e) {
			System.err.println("Connection refused. You need to initiate a Client 3 first.");
		}
	...
	void receivefromclient3_2() // to receive chunks from Client3
	{
		try {
			// create a socket to connect to the Client3
			requestSocket2 = new Socket("localhost", 9003);
			System.out.println("Connected to localhost Client 3 in port 9003 again for 2nd session.");
			...
			try {
				...
				System.out.println("Chunks received from Client3 in session 2.");
			}
	...
	public static int sendtoclient5() {
		return noofparts; // to know number of chunks
	}

	private static final int cPort = 9004; // The Client4 will be listening on this port number for request from Client5

	public static void main(String args[]) throws Exception {
		Client4 client = new Client4();
		client.receivefromserver();
		client.receivefromclient3();

		ServerSocket listener = new ServerSocket(cPort);
		System.out.println("The Client 4 is listening for request from Client 5. Please start Client 5.");
		int parts = client.sendtoclient5();
		int ClientNum = 1;
		try {
			new Handler1(listener.accept(), ClientNum, parts, Name).start();
			ClientNum++;
			client.receivefromclient3_2();
		} finally {
			listener.close();
		}

		System.out.println("All " + parts + " chunks received by Client4. Now starting to merge these chunks.");
		...
		System.out.println("You can find the merged file in Client4 Folder");
	}
	...
	private static class Handler1 extends Thread {
		...
		public Handler1(Socket connection, int no, int parts, String fileName) {
			...
			if (this.no == 1) {
				System.out.println("Client 5 Connected");
				...
				try {
					...
					System.out.println("Error, Client4 cannot send ");
				}
